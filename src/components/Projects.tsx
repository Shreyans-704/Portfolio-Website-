        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {projectsData.map((p, i) => (
            <motion.div
              ...
              className="group cursor-pointer relative flex flex-col p-8 rounded-[2rem] bg-white/[0.02] border border-white/10 backdrop-blur-md hover:bg-white/[0.04] hover:shadow-[0_0_40px_rgba(59,130,246,0.15)] hover:scale-[1.02] transition-all duration-500 overflow-hidden"
            >
              <a 
                href={p.githubUrl}
                target="_blank"
                ...
                className="absolute top-0 right-0 p-8 opacity-0 -translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 ease-out z-20"
                title="View GitHub Repository"
              >
                <ArrowRight className="w-5 h-5 text-blue-400 hover:text-blue-300 hover:scale-110 transition-transform" />
              </a>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-10 pointer-events-none">
              ...
              >
                <div className="p-8 md:p-12 overflow-y-auto no-scrollbar relative">
                  ...
                  <button 
                    onClick={() => setSelectedId(null)}
                    className="absolute top-8 right-8 p-3 rounded-full bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white transition-colors"
                  >
                    <X className="w-5 h-5" />
                  </button>
                  ...
                  {projectsData.map(p => p.id === selectedId && (
                    <div key={p.id} className="flex flex-col h-full">
                       <motion.span layoutId={`category-${p.id}`} className="text-sm font-semibold tracking-widest uppercase text-blue-500 mb-6 block">
                        {p.category}
                      </motion.span>
